...
function Navbar() {
    ...
    const getDataWithTitle = (query = null) => {
        axios.get(`http://localhost:2345/jobs?title=${query}`).then((res) => {
            setJobsData(res.data.jobs)
            console.log(res)
            return;
        })
    }
    const getDataWithLocation = (query = null) => {
        axios.get(`http://localhost:2345/jobs?location=${query}`).then((res) => {
            setJobsData(res.data.jobs)
            return;
        })
    }
    ...
    const handleJobs = () => {
        console.log(title)
        if (title) {
            getDataWithTitle(title);
            return;
        } else if (location) {
            getDataWithLocation(location);
            return;
        }
        return getData();
            ...
            <NavbarStyled>
                ...
                <SearchStyled>
                    ...
                    <div className="locationSelect">
                        ...
                        }} name="" style={{ border: "none", outline: "none" }} id="">
                    ...
                    <div>
                        <svg onClick={handleJobs} className="searchButton" width="280" height="60" viewBox="0 0 280 60" fill="none" xmlns="http://www.w3.org/2000/svg">
